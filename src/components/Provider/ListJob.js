...
export const ListJob = ({ job, applyJob }) => {
  const { userData } = useAuth();
  const isDisabled =
    job.appliedProviders &&
    job.appliedProviders.length > 1 &&
    job.appliedProviders.find((p) => p.userId === userData.userId);
          ...
          <div class="solution_card">
            ...
            <div class="solu_description">
              ...
              <p>
                ...
                {job.skills &&
                  job.skills.length > 0 &&
                  job.skills.map((j, i) => <Badge key={i}>{j}</Badge>)}
              </p>
              <button
                type="button"
                class="read_more_btn"
                onClick={() => navigate(`/job/provider/view/${job.jobId}`)}
              >
                Read More
              </button>
              <button
              ...
              >
                {" "}
                {isDisabled ? "Already Applied" : "Apply"}
              </button>
            </div>
          </div>
        </div>
