...
export const ListBuyerJobs = () => {
  ...
  const fetchUserListedJobs = async () => {
    try {
      setJobs([])
      setLoading(true)
      const q = query(
  ...
  React.useEffect(() => {
    currentUser && fetchUserListedJobs()
  }, [currentUser])
  ...
  return (
    <>
      <h3>Jobs created </h3>
      {jobs.length <= 0 ? (
        <h2>No jobs created. Want to create one ? </h2>
      ) : (
        jobs.map((j, i) => (
                ...
                <div class='solution_card'>
                  ...
                  <div class='solu_description'>
                    ...
                    <p>
                      ...
                      {j.skills &&
                        j.skills.length > 0 &&
                        j.skills.map((j, i) => <Badge key={i}>{j}</Badge>)}
                    </p>
                    <button
                      type='button'
                      class='read_more_btn'
                      onClick={() => navigate(`/job/buyer/view/${j.jobId}`)}
                    >
                      Read More
                    </button>
                    <button
                      type='button'
                      class='read_more_btn'
                      onClick={() => navigate(`/job/edit/${j.jobId}`)}
                    >
                      {' '}
                      Edit
                    </button>
          ...
          //     j.skills.map((j, i) => <Badge key={i}>{j}</Badge>)}
          // </p>
          // <button class="login__submit" onClick={() => navigate(`/job/edit/${j.jobId}`)}>
          //   Edit
          // </button>
          // <button class="login__submit" onClick={() => navigate(`/job/buyer/view/${j.jobId}`)}>
          //   View
          // </button>
          // <button class="login__submit" color='danger' onClick={() => deleteJobs(j.jobId)}>
          //   Delete
          // </button>
          // </Card>
        ))
